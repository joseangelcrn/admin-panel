...
@section('title','Admin - Asignar tareas a un usuario')
...
@section('content')
    ...
    @section('header','Asignar tareas a un usuario')
            ...
            <div class="col-lg-12 col-md-12">
                <form action="{{route('task.assign')}}" method="POST">
                    ...
                    <div class="form-group">
                        <select style="display: none;" id="task-selector" class="form-control"  name="task_id[]" multiple="multiple">
                            @foreach ($tasks as $task)
                                <option value="{{$task->id}}" data-name="{{$task->name}}" title="{{$task->description}}">{{$task->title}}</option>
                            @endforeach
                        </select>
                    </div>

                    <div class="form-group">
                        <button class="btn btn-success">Asignar tareas</button>
                    </div>
                </form>
            </div>
        </div>
...
@section('js')
    ...
    <script>
            var task_selector = $('#task-selector');

            // Call the dataTables jQuery plugin
            $(document).ready(function() {
                task_selector.multiselect({
